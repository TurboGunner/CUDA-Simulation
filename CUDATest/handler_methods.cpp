...
void CudaExceptionHandler(const cudaError_t cuda_status, const string& error_message) {
...
__host__ __device__ void CudaExceptionHandlerCross(const cudaError_t cuda_status, const char* error_message) {
    if (cuda_status != cudaSuccess) {
        printf("%s\n", error_message);
...
cudaError_t CopyFunction(const string err_msg, void* tgt, const void* src, const cudaMemcpyKind mem_copy_type, const cudaError_t error, const size_t size_alloc, const size_t element_alloc) {
    cudaError_t cuda_status = error;
    if (error == cudaSuccess) {
        cuda_status = cudaMemcpy(tgt, src, size_alloc * element_alloc, mem_copy_type);
