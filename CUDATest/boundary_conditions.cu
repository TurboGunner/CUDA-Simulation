...
__global__ void BoundaryConditions(int bounds, HashMap* c_map, uint3 size) {
	...
	if (x_bounds == size.x - 4 && y_bounds == size.y - 4) { //Correction for check due to bounded thread changes
		...
		c_map->Put(IndexPair(bound, 0, bound).IX(length), .33f * //Max X, Min Y, Max Z
		...
		c_map->Put(IndexPair(0, bound, bound).IX(length), .33f * //Max X, Min Y, Max Z
		...
		c_map->Put(IndexPair(bound, bound, bound).IX(length), .33f * //Max X, Max Y, Max Z
