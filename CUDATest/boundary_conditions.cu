...
__global__ void BoundaryConditions(int bounds, HashMap<float>* c_map, uint3 size) {
	...
	(*c_map)[IndexPair(x_bounds, y_bounds, 0).IX(length)] = bounds == 3 ? -(*c_map)[IndexPair(x_bounds, y_bounds, 1).IX(length)] : (*c_map)[IndexPair(x_bounds, y_bounds, 1).IX(length)];
	(*c_map)[IndexPair(x_bounds, y_bounds, bound).IX(length)] = bounds == 3 ? -(*c_map)[IndexPair(x_bounds, y_bounds, bound - 1).IX(length)] : (*c_map)[IndexPair(x_bounds, y_bounds, bound - 1).IX(length)];

	(*c_map)[IndexPair(y_bounds, 0, x_bounds).IX(length)] = bounds == 2 ? -(*c_map)[IndexPair(x_bounds, 1, y_bounds).IX(length)] : (*c_map)[IndexPair(x_bounds, 1, y_bounds).IX(length)];
	(*c_map)[IndexPair(y_bounds, bound, x_bounds).IX(length)] = bounds == 2 ? -(*c_map)[IndexPair(x_bounds, bound - 1, y_bounds).IX(length)] : (*c_map)[IndexPair(x_bounds, bound - 1, y_bounds).IX(length)];

	(*c_map)[IndexPair(0, y_bounds, x_bounds).IX(length)] = bounds == 1 ? -(*c_map)[IndexPair(1, x_bounds, y_bounds).IX(length), x_bounds] : (*c_map)[IndexPair(1, x_bounds, y_bounds).IX(length)];
	(*c_map)[IndexPair(bound, y_bounds, x_bounds).IX(length)] = bounds == 1 ? -(*c_map)[IndexPair(bound - 1, x_bounds, y_bounds).IX(length)] : (*c_map)[IndexPair(bound - 1, x_bounds, y_bounds).IX(length)];
...
cudaError_t BoundaryConditionsCuda(int bounds, HashMap<float>* map, const uint3& length) {
