...
__device__ Camera::Camera(Vector3D lookfrom, Vector3D lookat, Vector3D vup, float vfov, float aspect, float aperture, float focus_dist) { // vfov is top to bottom in degrees, Good
...
__device__ Ray Camera::GetRay(float s, float t, curandState* rand_state) { //Good
