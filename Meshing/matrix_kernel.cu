...
__host__ cudaError_t Matrix::TransposeGPU(Matrix* matrix, Matrix* output) {
    cudaError_t cuda_status = cudaSuccess;

    dim3 blocks, threads;

    blocks = dim3(1, 1);
    threads = dim3(matrix->rows, matrix->columns);
...
__global__ void MultiplyKernel(Matrix* matrix_A, Matrix* matrix_B, Matrix* output) {
    ...
    if (y_bounds < output->rows && x_bounds < output->columns) {
        ...
        for (int k = 0; k < matrix_A->columns; ++k) {
            Pvalue += matrix_A->Get(y_bounds * matrix_A->columns + k) * matrix_A->Get(k * matrix_B->columns + x_bounds);
        }
